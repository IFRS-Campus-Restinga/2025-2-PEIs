...
export default function Cursos() {
  ...
  async function handleDownload(cursoId, arquivoNome) {
    const response = await DBCURSOS.get(`/${cursoId}/download/`, { responseType: 'blob' });
          ...
          <tbody>
            {cursos.map((curso) => (
              <tr key={curso.id}>
                ...
                <td>
                  {curso.arquivo ? (
                    <div className="download-wrapper" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <span>{curso.arquivo_nome}</span>
                      <button
                        type="button"
